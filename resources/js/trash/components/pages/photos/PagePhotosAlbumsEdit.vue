...
export default {
    ...
    async beforeRouteEnter(to, from, next) {
        const data = (await axios.get('/api/photo_albums/' + to.params.album_id)).data;
        next(vm => {
            vm.album = data;
        });
    },
    methods: {
        async update() {
            ...
            const { data } = await axios.put('/api/photo_albums/' + this.album.id, querystring.stringify(this.album));
